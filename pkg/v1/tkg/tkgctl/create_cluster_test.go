...
var _ = Describe("Unit tests for - TKGS Cluster - cluster_tkgs.yaml as input file for 'tanzu cluster create -f cluster_tkgs.yaml' use case", func() {
	var (
		tkgctlClient  *tkgctl
		tkgClient     = &fakes.Client{}
		bomClient     = &fakes.TKGConfigBomClient{}
...
var _ = Describe("Unit tests for feature flag (config.FeatureFlagPackageBasedLCM) and featureGate for clusterclass - TKGS : Vsphere provider, devcc:", func() {
	...
	Context("Creating clusters for TKGs", func() {
		...
		It("When Feature constants.CCFeature is disabled in featuregate, return error", func() {
		...
		It("When featuregate api throws error", func() {
